...
int main(int argc, char** argv) {
  ...
  OutPacketBuffer::maxSize = 100000; // bytes  h265
  ...
  *env << "LIVE555 Proxy Server\n"
  ...
  for (i = 1; i < argc; ++i) {
    ...
    char streamName[30];
    if (argc == 2) {
      sprintf(streamName, "%s", "proxyStream"); // there's just one stream; give it this name  1
    } else {
      sprintf(streamName, "proxyStream-%d", i); // there's more than one stream; distinguish them by name  >1
    }
	 // 新建转发SESSION  
    ...
    // proxiedStreamURL是代理的源rtsp地址字符串，streamName表示代理后的ServerMediaSession的名字
