...
#if 0
// new DynamicRTSPServer::createNew lihai
DynamicRTSPServer*  
DynamicRTSPServer::createNew(UsageEnvironment& env, Port ourPort,  
                 UserAuthenticationDatabase* authDatabase,  
                 t_h265_Buffer *p_265buf,  
                 unsigned reclamationTestSeconds)   
{  
    int ourSocket = setUpOurSocket(env, ourPort);  
    if (ourSocket == -1) return NULL;  
    ...
    newDynamicRTSPServer->fPTsBuf = p_265buf;  
    return newDynamicRTSPServer;  
}  
  	
#endif

/* 首先建立socket，然后在调用DynamicRtspServer的构造函数，
 * DynamicRtspServer继承RTSPServerSupportingHTTPStreaming类; 
 * RTSPServerSupportingHTTPStreaming类又继承RTSPServer类；
...
DynamicRTSPServer::DynamicRTSPServer(UsageEnvironment& env, int ourSocket,
				     ...
				     UserAuthenticationDatabase* authDatabase, 
				     unsigned reclamationTestSeconds)
  : RTSPServerSupportingHTTPStreaming(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds), fPTsBuf(NULL){
}
